...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if s := req.URL.Scheme; s != "http" && s != "https" {
		...
		return nil, errors.New("unsupported protocol scheme " + s)
	}
	if req.URL.Host == "" {
		if req.Body != nil {
			req.Body.Close()
...
func (t *Transport) getConn(req *http.Request, cm connectMethod) (*persistConn, error) {
	if pc := t.getIdleConn(cm); pc != nil {
		glog.Infof("Fetched idle connection %v", pc)
...
func (pc *persistConn) roundTrip(req *transportRequest) (resp *http.Response, err error) {
	...
	if req.ProtoAtLeast(1, 1) && req.Body != nil {
...
func (pc *persistConn) markBroken() {
	pc.lk.Lock()
	defer pc.lk.Unlock()
	glog.Infof("Marked connection %v broken", pc)
	pc.broken = true
}
